...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    // Add money to the till only after we make sure the customer has enough money to pay for the item
    cashTill[3] += twenties;
    cashTill[2] += tens;
    cashTill[1] += fives;
    cashTill[0] += ones;        
    ...
    int[] cashTracker = new int[] { 0, 0, 0, 0 };
    ...
    if (changeNeeded > 0) 
    {
        // Place change back into the till
        for (int i = 3; i >=0; i--)
        {
            while(cashTracker[i] > 0) 
            {
                cashTracker[i]--;
                cashTill[i]++;
            }
        ...
        // Refund Customer Money by removing from till
        cashTill[3] -= twenties;
        cashTill[2] -= tens;
        cashTill[1] -= fives;
        cashTill[0] -= ones;        
